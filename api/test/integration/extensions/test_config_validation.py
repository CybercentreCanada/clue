...
from clue.extensions.config import BaseExtensionConfig, Modules


class CustomTestConfig(BaseExtensionConfig):
...
def test_validation(caplog):
    ...
    result = CustomTestConfig.initialize_extension_configuration(  # type: ignore[operator]
    ...
    result = CustomTestConfig.initialize_extension_configuration(  # type: ignore[operator]
        {
            "name": "test",
            "modules": {"obo_module": True},
        }
    )

    assert result["modules"]["obo_module"] == "test.obo:get_obo_token"

    result = CustomTestConfig.initialize_extension_configuration(  # type: ignore[operator]
        {
            "name": "test",
            "modules": {"obo_module": "test.example:get_obo_token"},
        }
    )

    assert result["modules"]["obo_module"] == "test.example:get_obo_token"
