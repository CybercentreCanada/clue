...
const Fetcher: FC<FetcherProps> = React.memo(({ type, value, classification, fetcherId, slotProps = {} }) => {
  ...
  if (fetchCompleted) {
    if (!fetcherId) {
      ...
      console.warn("Invalid fetcher id. Must be in the format '<plugin_id>.<fetcher_id>'. Component will not render.");
      return null;
    ...
    if (fetchers[fetcherId]?.format === 'status') {
      return (
        <Skeleton
          ...
          sx={[
            { borderRadius: '16px' },
            ...(Array.isArray(slotProps.skeleton?.sx) ? slotProps.skeleton?.sx : [slotProps.skeleton?.sx])
          ]}
        />
      );
    } else {
      return <Skeleton variant="rounded" height="325px" width="300px" {...(slotProps.skeleton ?? {})} />;
    }
  }
  ...
  if (result?.format === 'status') {
    return <StatusChip data={result.data} {...(slotProps.chip ?? {})} />;
  }
