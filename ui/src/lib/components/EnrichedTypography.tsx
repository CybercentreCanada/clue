import type { PopoverProps, PopperProps, StackProps, TypographyProps } from '@mui/material';
...
const EnrichedTypography: FC<EnrichedTypographyProps & TypographyProps> = ({
  type: _type,
  value,
  ...
  return (
    ...
    >
      ...
      />
      {children ?? (
        <FrequencyText id={`enriched-${type}-value`} {...otherProps} annotations={annotations} value={value} />
      )}
      <AssessmentIcon
        value={{ type, value, classification }}
        annotations={annotations}
      ...
      {!loading && useDetailsIcon && (
        <Tooltip title={t('details.open')}>
          <IconButton size="small" onClick={clicker} sx={{ color: theme.palette.action.disabled }}>
...
export default memo(EnrichedTypography);
