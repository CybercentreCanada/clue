...
describe('Configs API', () => {
  ...
  describe('uri', () => {
    it('should construct the correct URI for configs endpoint', () => {
      ...
      expect(vi.mocked(joinUri)).toHaveBeenCalledWith('/api/v1/lookup', 'types');
      expect(result).toBe('/api/v1/lookup/types');
    });
  });
  ...
  describe('get', () => {
    it('should call hget with correct parameters when no config provided', async () => {
      ...
      expect(vi.mocked(hget)).toHaveBeenCalledWith('/api/v1/lookup/types', null, undefined);
    ...
    it('should call hget with provided config', async () => {
      ...
      expect(vi.mocked(hget)).toHaveBeenCalledWith('/api/v1/lookup/types', null, config);
      expect(result).toEqual({
        example: ['ip'],
        potato: ['ip', 'sha256']
      });
