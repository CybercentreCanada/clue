...
async def test_server_defaults_invalid_port(monkeypatch):
    monkeypatch.setattr("clue_mcp.config.MCPSettings.PORT", "not-a-port")
    sys.modules.pop("clue_mcp.server", None)

    with (
        patch("mcp.server.fastmcp.FastMCP") as mock_fast_mcp,
        patch("clue_mcp.api.ClueApiClient") as mock_api_client,
        patch("clue_mcp.tools.register_tools") as mock_register_tools,
        patch("clue_mcp.prompts.register_prompts") as mock_register_prompts,
    ):
        api_client = mock_api_client.return_value
        api_client.aclose = AsyncMock()
        server = importlib.import_module("clue_mcp.server")
