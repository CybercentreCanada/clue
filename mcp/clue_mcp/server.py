...
from .config import AUTH, CLUE_API, MCPSettings
...
def _with_api_client_lifespan(app: Starlette, client: ClueApiClient) -> Starlette:
    original_lifespan = app.router.lifespan_context

    @asynccontextmanager
    async def lifespan(app: Starlette) -> AsyncGenerator[None, None]:
        limits = httpx.Limits(
            max_connections=CLUE_API.MAX_CONNECTIONS,
            max_keepalive_connections=CLUE_API.MAX_KEEPALIVE_CONNECTIONS,
            keepalive_expiry=CLUE_API.KEEPALIVE_EXPIRY,
        )
        try:
