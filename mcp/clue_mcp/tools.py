...
def register_tools(mcp, api_client: ClueApiClient):
    ...
    @mcp.tool(name="craft_clue_url")
    def craft_clue_url(record: dict[str, Any]) -> str:
        """Build the Clue UI URL for a hit or event returned by Clue.
        ...
        Returns:
            str: URL that opens the record in the Clue web interface.
        ...
        record_id = clue.get("id") if isinstance(clue, dict) else None
        if not isinstance(record_id, str) or not record_id.strip():
            raise ValueError("clue.id is required and cannot be empty.")

        record_id = record_id.strip()
        if _contains_escape_characters(record_id):
            raise ValueError("record.clue.id cannot contain control characters or path separators.")

        return f"{CLUE_UI.BASE_URL}/{record_type}s/{record_id}"

    @mcp.tool(name="whoami", description="Get information about the current user")
    async def whoami() -> WhoAmIResponse:
        """Return identity details for the authenticated Clue user.
        ...
        Returns:
            ...
            information for the current Clue user.
        """
    ...
    async def list_assigned_hits() -> list[dict[str, Any]]:
        ...
        Returns:
            list[dict[str, Any]]: Hits returned by the Clue API for the
    ...
    async def get_field_values(field: str) -> dict[str, int]:
        """Return the distinct values and their hit counts for a specific Clue hit field.

        Use this tool when you know a field name but are unsure which values it
        accepts. For example, call it with ``clue.escalation`` or
        ``clue.assessment`` before building a Lucene query that filters on
        those fields.

        Args:
            field: Exact field name to inspect, such as ``clue.escalation``
                or ``clue.assessment``.
        ...
        field_values = await api_client.call(
            ...
            # matching document to return any distinct values. Using clue.id:*
            # matches every hit so we always get the full value distribution.
            params={"query": r"clue.id:*"},
        )
    ...
    async def get_hit_fields() -> dict[str, Any]:
        """Return all searchable fields available on Clue hits.
        ...
        Returns:
            ...
            example ``{"clue.assignment": {"type": "keyword", "indexed": true,
            "stored": false}, ...}``.
    ...
    async def lucene_query(
        ...
        Common Clue fields:
            clue.id: Exact hit identifier.
            clue.assignment: Assigned user or queue.
            clue.escalation: Escalation state such as ``alert``.
            clue.assessment: Analyst assessment such as
                ``false-positive``.
            clue.analytic: Analytic name.
            clue.score: Numeric hit score.
            clue.outline.indicators: Indicator values extracted from the
                hit.
            clue.outline.target: Target values associated with the hit.
            clue.outline.threat: Threat values associated with the hit.
            event.created: Hit event timestamp, commonly used in time ranges.

        Examples:
            clue.assignment:user
            clue.analytic:"Password Checker"
            clue.escalation:alert AND event.created:[now-7d TO now]
            clue.score:[50 TO 100] AND NOT clue.assessment:false-positive
            clue.outline.indicators:(example.com OR 1.2.3.4)
        ...
        **Reducing response size with** ``fl``:
        ...
        ``clue.id`` is always returned regardless of ``fl``.

        Examples:
            fl="clue.id,clue.assignment,clue.status"
            fl="clue.id,clue.analytic,clue.detection,clue.assessment"
            fl="clue.id,clue.outline.indicators,clue.outline.threat"
        ...
        victim was 8.8.8.8" should request only detection (and ``clue.id``)
        ...
        Args:
            ...
            sort: Optional Clue sort expression, such as
            ...
            fl: Required comma-separated list of field names to include in each
                ...
                user. ``clue.id`` is always added automatically if missing.

        Returns:
            ClueResponse: Structured search results containing the total count,
        ...
        # clue.id is always included so hits remain identifiable.
        requested = list(dict.fromkeys(f.strip() for f in fl.split(",") if f.strip()))
        if "clue.id" not in requested:
            requested.insert(0, "clue.id")
    ...
    async def get_label_set_options() -> list[str]:
        """Return the valid hit label categories supported by Clue.
        ...
        for field in cached_hit_fields:
            # Only keep hit label fields such as clue.labels.generic.
            if not field.startswith("clue.labels."):
                continue

            # Strip the common prefix so the caller gets just the label set
            # name, for example "generic" instead of "clue.labels.generic".
            label = field.replace("clue.labels.", "")
    ...
    async def add_label_to_hit(hit_id: str, labels_name: list[str], label_set: str) -> list[str]:
        ...
        Args:
            hit_id: Clue hit identifier to update.
        ...
        return data.get("clue", {}).get("labels", {}).get(label_set, [])
            ...
            "pivots": [
                {
                    ...
                    "format": "link",
                    "mappings": [{"key": "ioc", "field": "clue.outline.indicators"}],
                }
            ],
        }
        """
    ...
    async def create_dossier_for_hit(hit_id: str, leads: list[dict]) -> dict:
        ...
        dossier. It appends the supplied leads to the hit's ``clue.dossier``
        ...
        operations = [["APPEND", "clue.dossier", lead] for lead in leads]
        return await api_client.call(
            body=operations,
            method="PUT",
