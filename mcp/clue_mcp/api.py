...
class ClueApiClient:
    """HTTP client for communicating with the Clue REST API.

    Handles token exchange, request construction, and response unwrapping
    for all MCP tool calls that need to reach the Clue backend.
    """

    def __init__(
        self,
        base_url: str = CLUE_API.BASE_URL,
        auth_provider: AuthProvider | None = None,
        timeout: float = CLUE_API.TIMEOUT,
        client: httpx.AsyncClient | None = None,
    ) -> None:
        """Initialise the API client.

        Args:
            base_url: Base URL of the Clue API, including the version prefix.
            auth_provider: Provider used to exchange the MCP user token for a
                Clue-compatible bearer token. Defaults to a new
    ...
    async def call(
        ...
        Args:
            user_access_token: The verified MCP user token used to obtain a
                Clue bearer token via the configured ``AuthProvider``.
